...
export default function MessagesPageClient({
  ...
  const [viewMode, setViewMode] = useState<"list" | "chat">(initialThreadId ? "chat" : "list");
  ...
  useEffect(() => {
    // Initial fetch of messages if activeThreadId is set
    if (activeThreadId) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const optimisticMsg: Message = {
      id: `opt-${Date.now()}`,
      sender_id: "me", // Identifier for self
              ...
              filteredThreads.map((thread) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full group gap-4 relative duration-200 transition-all flex border-none p-4 text-left cursor-pointer ${
                      isSelected 
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 relative">
                      <div className={`justify-center rounded-2xl size-12 items-center text-white shadow-sm flex font-bold ${getAvatarStyle(otherPart.email)}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="items-baseline flex mb-1 justify-between">
                        <h3 className="type-ui truncate font-semibold text-text group-hover:text-primary transition-colors">
                          {otherPart.name}
                        </h3>
                      ...
                      <p className="truncate text-muted mb-1 type-caption text-xs">
                        {otherPart.headline}
                      </p>
                    ...
                    {hasUnread && (
                      <div className="flex-shrink-0 flex justify-center items-center">
                        <span className="size-5 justify-center text-xs items-center text-white rounded-full shadow-lg shadow-primary/20 bg-primary flex font-semibold">
                          {thread.unread_count}
                        </span>
                      </div>
                    )}
        ...
        <div className={`bg-bg/20 h-full flex-1 transition-all duration-300 flex flex-col ${
          ...
          {activeThread ? (
            <>
              ...
              <div className="border-b border-border gap-4 items-center flex p-4 bg-surface justify-between">
                <div className="flex gap-3 items-center min-w-0">
                  ...
                  <div className={`flex-shrink-0 justify-center rounded-2xl items-center size-11 text-white shadow-sm flex font-bold ${getAvatarStyle(activeThread.other_participant.email)}`}>
                    {activeThread.other_participant.name.charAt(0).toUpperCase()}
                  </div>

                  <div className="min-w-0">
                    <h2 className="text-text type-ui truncate font-semibold">
                ...
                <div className="gap-2 flex items-center">
                  <span className="py-1 text-success px-2.5 bg-success-bg rounded-full type-badge text-xs">
                    Online
                  </span>
                </div>
              </div>
                ...
                {loadingMessages && messages.length === 0 ? (
                  ...
                  <>
                    <div className="text-center py-4">
                      <span className="bg-border/30 px-3 py-1.5 rounded-full type-badge text-muted text-xs">
                    ...
                    {messages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`break-word rounded-3xl text-sm shadow-sm p-4 ${
                            isMe
            ...
            <div className="justify-center h-full items-center text-center p-6 flex text-muted flex-col">
              ...
              <h2 className="text-text type-h2 mb-1 font-semibold text-xl">Your Conversation Canvas</h2>
              <p className="type-ui max-w-sm text-sm">Select a thread from the side panel to view messages or start corresponding with hiring stakeholders.</p>
            </div>
          )}
        </div>
