...
import { Notification, SavedSearchAlert, NotificationPreferences } from "@/types/notification";

interface AlertsPageClientProps {
  initialPreferences: NotificationPreferences;
  initialAlerts: SavedSearchAlert[];
...
export default function AlertsPageClient({
  ...
  const handleToggleAlertActive = async (alert: SavedSearchAlert) => {
    const originalAlerts = [...alerts];
    const updatedAlerts = alerts.map(a => a.id === alert.id ? { ...a, is_enabled: !alert.is_enabled } : a);
    ...
    try {
      ...
      if (res.error) {
        ...
        toast.success(!alert.is_enabled ? "Alert activated!" : "Alert paused");
      }
    } catch (err) {
      ...
      (t) => (
        ...
        >
          ...
          <button
            ...
            className="text-primary hover:text-primary/80 text-sm font-semibold whitespace-nowrap focus:outline-none cursor-pointer"
          >
            Undo
          </button>
        </div>
  ...
  const handleRunSearch = (alert: SavedSearchAlert) => {
    // Run search based on keywords & location
    const query = alert.keywords || "";
                  ...
                  return (
                    ...
                    >
                      <div className={`justify-center w-10 shrink-0 items-center flex h-10 rounded-xl border ${isUnread ? "text-primary border-primary/20 bg-primary/10" : "bg-bg text-muted border-border/30"}`}>
                        <Icon name={getNotificationIcon(n.notification_type)} size={20} aria-hidden="true" />
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <Text variant="body-sm" color={isUnread ? "default" : "muted"} className="mt-1 block">
                          {n.message || "Opportunity listing update."}
                        </Text>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                        ...
                        <Button
                          ...
                          aria-label={alert.is_enabled ? "Pause alert" : "Activate alert"}
                        >
                          {alert.is_enabled ? <BellOff size={16} /> : <BellRing size={16} />}
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
