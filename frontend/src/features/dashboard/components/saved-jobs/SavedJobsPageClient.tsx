...
export default function SavedJobsPageClient({ initialJobs }: SavedJobsPageClientProps) {
  ...
  const handleUnsave = async (jobId: string) => {
    ...
    // Register active unsave state
    activeTimers.current[jobId] = {
      undone: false,
      timer: setTimeout(async () => {
        const state = activeTimers.current[jobId];
        if (state && !state.undone) {
          try {
            const res = await unsaveJobAction(jobId);
            if (res.error) {
              toast.error("Failed to unsave job. Restored.");
              // Roll back
              setJobs(prev => {
            ...
            toast.error("Failed to unsave job due to network connection issues. Restored.");
            setJobs(prev => {
              if (prev.some(j => j.id === jobId)) return prev;
              return [...prev, jobToRestore];
            });
        ...
        delete activeTimers.current[jobId];
      }, 5000)
    };
            ...
            <div className="flex gap-3 items-start justify-between">
              <div className="flex gap-3 items-center min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="type-ui truncate transition-colors text-text group-hover:text-primary font-semibold">
                    {job.title}
                  </h4>
                  <p className="truncate text-muted type-caption text-xs">
