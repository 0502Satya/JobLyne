...
function CompanyMessagesContent() {
  ...
  const handleSelectThread = (threadId: string) => {
    ...
    router.replace(`/company/messages?${params.toString()}`);
  };
    ...
    <div className="text-text bg-bg transition-colors flex min-h-screen flex-col">
      ...
      <header className="border-b border-border px-6 py-4 items-center transition-all sticky z-50 flex top-0 bg-surface justify-between md:px-12">
        <div className="flex gap-6 items-center">
          ...
          <span className="px-3 text-emerald-500 hidden py-1.5 bg-emerald-500/10 rounded-full type-badge md:inline-block">Corporate Workspace</span>
        </div>
        <div className="gap-4 flex items-center">
          ...
          <div className="flex gap-3 items-center">
            <div className="justify-center rounded-2xl bg-gradient-to-tr items-center size-11 text-white shadow-emerald-500/20 shadow-lg to-[#059669] flex from-emerald-500">
              HQ
            </div>
            <button 
      ...
      <main className="w-full mx-auto max-w-7xl flex-1 box-sizing space-y-6 p-6 md:p-12">
        ...
        <div className="space-y-2">
          <h1 className="text-text type-h1">Corporate Communications</h1>
          <p className="type-label">Engage with applicants directly. Discuss skillsets, background details, and organize scheduling.</p>
        </div>
            ...
            <div className="divide-y divide-border/40 flex-1 overflow-y-auto">
              {loadingThreads ? (
                ...
                <div className="justify-center h-full items-center text-center p-6 flex text-muted flex-col">
                  <MessagesSquare className="text-muted/60 mb-2" size={24} aria-hidden="true" />
                  <p className="type-ui">No discussions started</p>
                  <p className="mt-1 max-w-[220px] text-xs text-muted/80">Open your sourcing candidate pool in the Dashboard and click "Chat" to engage.</p>
                </div>
              ) : (
            ...
            {activeThread ? (
              <>
                <div className="border-b border-border gap-4 items-center flex p-4 bg-surface justify-between">
                  <div className="flex gap-3 items-center min-w-0">
                    <button
                      ...
                      className="md:hidden flex-shrink-0 text-text justify-center border-border min-h-[44px] items-center flex size-10 rounded-xl border hover:bg-bg"
                            ...
                            <div className="px-1.5 gap-1.5 items-center flex mt-1">
                              ...
                              {isMe && (
                                <span className="flex items-center">
                                  {msg.isOptimistic ? (
                                    <Clock className="text-muted" size={14} aria-hidden="true" />
                                  ) : (
                                    <CheckCheck className={msg.is_read ? "text-primary" : "text-muted"} size={14} aria-hidden="true" />
                                  )}
                                </span>
                              )}
                            </div>
                ...
                >
                  <input
                    type="text"
                    placeholder="Type a corporate message to candidate..."
                    value={messageInput}
                  ...
                  <button
                    ...
                    className="flex-shrink-0 justify-center min-h-[48px] rounded-2xl size-12 items-center transition-all text-white min-w-[48px] shadow-primary/20 bg-primary flex shadow-lg hover:scale-105 hover:bg-primary/95 active:scale-95 disabled:shadow-none disabled:opacity-50 disabled:scale-100"
              ...
              <div className="justify-center h-full items-center text-center p-6 flex text-muted flex-col">
                <MessageCircle className="mb-4 text-muted/40" size={32} aria-hidden="true" />
                <h2 className="text-text type-h2 mb-1">Corporate Inbox</h2>
                <p className="type-ui max-w-sm">Select an applicant discussion from the side panel to view messages or sync hiring status.</p>
              </div>
            )}
          </div>
