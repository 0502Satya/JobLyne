    ...
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen font-display transition-colors flex flex-col">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 md:px-20 py-4 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <Link href="/recruiter/auth/signup" className="text-sm font-bold text-primary hover:underline">Register Now</Link>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center py-12 px-4 relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 md:p-10 shadow-2xl transition-all relative z-10">
          ...
          <div className="space-y-6">
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink mx-4 text-slate-400 text-[10px] font-black uppercase tracking-widest italic">or manually</span>
              <div className="flex-grow border-t border-slate-200 dark:border-slate-800"></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                <input type="checkbox" className="rounded text-primary focus:ring-primary cursor-pointer" id="remember-recruiter" />
                <label htmlFor="remember-recruiter" className="text-xs text-slate-600 dark:text-slate-400 cursor-pointer italic">Keep me signed in for 30 days</label>
              </div>
        ...
        <div className="mt-10 flex gap-6 text-[11px] text-slate-400 font-medium relative z-10 transition-colors">
          <Link href="#" className="hover:text-primary transition-colors cursor-pointer">Support Resource</Link>
          <Link href="#" className="hover:text-primary transition-colors cursor-pointer">Security Protocol</Link>
        </div>
      </main>
