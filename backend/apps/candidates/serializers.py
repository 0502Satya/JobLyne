...
class CandidateProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'nationality', 'preferred_company_size', 'open_to_relocation', 'open_to_international', 'privacy_settings',
    ...
    def get_completeness(self, obj):
        return calculate_profile_completeness(obj)

    def get_skills(self, obj):
        return [link.skill.name for link in obj.job_seeker_skills_job_seeker.all() if link.skill]

    def validate_resume_file_url(self, value):
        if value:
            val_lower = value.lower().strip()
            if val_lower.startswith('javascript:'):
                raise serializers.ValidationError("Invalid URL protocol.")
            if len(value) > 2048:
                raise serializers.ValidationError("URL length exceeds limit.")
        return value
    ...
    def update(self, instance, validated_data):
        ...
        cache.delete(f'profile_completeness:{instance.user_id}')
